/**
 * @dsh/cron — browser half (P1 scheduled-task UI).
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			// Primary action (save/create) must not depend on `--dsw-alias-accent`,
			// which is not guaranteed to be inherited into the overlay's scoped
			// theme; use the always-resolved base tokens + weight so the button
			// is never rendered invisible (white-on-transparent).
			".dshc-textButtonPrimary{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-fill-l2);border-color:var(--dsw-alias-border-l2);font-weight:600}",
			...
			".dshc-drawerBody{flex:1;overflow:hidden;min-height:0;display:flex;flex-direction:column}",
			".dshc-formBody{flex:1;overflow:auto;min-height:0;padding:14px 16px;display:flex;flex-direction:column;gap:14px}",
			...
			".dshc-saveRow{flex:none;display:flex;align-items:center;gap:8px;justify-content:flex-end;padding:10px 16px 12px;border-top:1px solid var(--dsw-alias-border-l2);background:var(--dsw-specific-menu)}",
		...
		const tagId = "@dsh/cron/cronlist.css";
		...
		function JobForm({ initial, mode, timezone, t, onSubmit, onCancel, onDelete, submitting }) {
			...
			const [permission, setPermission] = react.useState((initial.target && initial.target.permissionMode) || "");
			...
			const submit = async () => {
				...
				// Always send the permission field: a value pins the mode, and
				// null explicitly clears it back to the deployment default
				// (omitting it would make PATCH keep the current mode).
				target.permissionMode = permission.length > 0 ? permission : null;
				const payload = {
					prompt: prompt.trim(),
			...
			return react_jsx_runtime.jsx(react.Fragment, {
				children: [
					...
					react_jsx_runtime.jsx(Field, {
						...
						hint: t("field.permission.hint"),
						children: react_jsx_runtime.jsx("select", {
							className: "dshc-sel",
							value: permission,
							onChange: (e) => setPermission(e.target.value),
							children: [
								react_jsx_runtime.jsx("option", { value: "", children: t("permission.none") }),
								react_jsx_runtime.jsx("option", { value: "read-only", children: t("permission.read-only") }),
								react_jsx_runtime.jsx("option", { value: "workspace-write", children: t("permission.workspace-write") }),
								react_jsx_runtime.jsx("option", { value: "danger-full-access", children: t("permission.danger-full-access") })
							]
						})
					}),
					react_jsx_runtime.jsx(Field, {
		...
		function CronOverlay({ t }) {
			...
			return react_jsx_runtime.jsx("div", {
				className: "dshc-page",
				"data-shell-overlay-page": "@dsh/cron",
				children: [
					react_jsx_runtime.jsx("div", {
		...
		const zh = {
			...
			"field.project.add.placeholder": "输入绝对路径，如 /Users/…/repos/my-project",
			"field.project.add.submit": "添加",
			"field.model": "模型 / 推理",
			"field.model.hint": "留空使用默认；推理可选 off / high / max",
			"effort.none": "推理默认",
			"field.permission": "权限模式",
			"field.permission.hint": "留空继承默认权限；只读 / 工作区写 / 全部访问",
			"permission.none": "默认",
			"permission.read-only": "只读",
			"permission.workspace-write": "工作区写",
			"permission.danger-full-access": "全部访问",
			"field.frequency": "频率",
		...
		const en = {
			...
			"field.project.add.placeholder": "Absolute path, e.g. /Users/…/repos/my-project",
			...
			"field.permission": "Permission mode",
			"field.permission.hint": "Leave empty to inherit the default; read-only / workspace write / full access",
