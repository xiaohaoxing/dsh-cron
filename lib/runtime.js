...
export class CronRuntime {
  ...
  async #joinAgentPreset(agentCtx) {
    const marker = process.env.DSH_CRON_PRESET_MARKER ?? '/tmp/dsh-cron-preset-join.log'
    try {
      const presets = this.#ctx.agentPresets
      if (presets === undefined) {
        appendFileSync(marker, `${new Date().toISOString()} setup-called agentPresets=undefined session=${agentCtx?.session?.id ?? '?'}\n`)
        return
      }
      await presets.mount(agentCtx)
      appendFileSync(marker, `${new Date().toISOString()} setup-called agentPresets=OK mount-ok session=${agentCtx?.session?.id ?? '?'}\n`)
    } catch (error) {
      appendFileSync(marker, `${new Date().toISOString()} setup-ERROR ${error instanceof Error ? error.message : String(error)}\n`)
      throw error
    }
  }
