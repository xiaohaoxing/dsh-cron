  ...
  async #joinAgentPreset(agentCtx) {
    ...
    try {
      ...
      if (presets === undefined) {
        appendFileSync(marker, `${new Date().toISOString()} setup-called agentPresets=undefined\n`)
        return
      }
      await presets.mount(agentCtx)
      appendFileSync(marker, `${new Date().toISOString()} setup-called agentPresets=OK mount-ok\n`)
    } catch (error) {
