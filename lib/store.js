...
/**
 * The sandbox permission modes the harness enforces (mirrors
 * `SANDBOX_MODES` in `@deepseek-ai/dsh-sandbox-policy`):
 *   - read-only            — no file modification through the sandbox
 *   - workspace-write      — write inside the session workspace
 *   - danger-full-access   — unrestricted file access
 * A job pins one of these on its run sessions via `target.permissionMode`;
 * when absent, run agents inherit the deployment default.
 */
...
export function validateJob(value) {
  ...
  return {
    ...
    target: {
      ...
      ...target.permissionMode != null ? { permissionMode: requireEnum(target.permissionMode, SANDBOX_MODES, 'job.target.permissionMode') } : {},
    },
    notification,
    enabled,
