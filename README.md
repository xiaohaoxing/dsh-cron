...
- **执行**：两种 target——
  - `new-chat`（默认）：每次运行开独立会话 `cron-<jobId>-<occurrence>`，可绑定 project 与 provider/model/reasoningEffort
...
- 详情抽屉：prompt 编辑、运行于（new/existing chat + 会话 ID）、项目（**工作区选择器**，可手动添加）、模型 + 推理、频率（cron 输入 + 预设 + 实时预览）、时区、通知（当前仅会话内）、运行历史
